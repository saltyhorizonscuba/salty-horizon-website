...
5 pages « produit » en anglais, chacune dupliquée nativement en `/fr/` et `/es/` (voir section 5), plus un blog anglais-only. **Mise à jour 2026-07-21** : le blog compte maintenant 7 articles (2 nouveaux ajoutés le 2026-07-21 : « The Top 10 Things to Do in Tamarindo » et « The 10 Best Beaches Near Tamarindo », tous deux avec schéma `BlogPosting`+`FAQPage`+`BreadcrumbList`, liens internes vers les pages produit). **Mise à jour 2026-07-23** : 2 nouvelles pages dédiées créées, **anglais uniquement pour l'instant** (pas encore de `fr`/`es`, donc pas de balises hreflang sur ces 2 pages — à ajouter si/quand les traductions sont faites) : `catalina-islands-diving.html` et `bat-islands-diving.html`, chacune avec schéma `WebPage`+`BreadcrumbList`+`FAQPage`, remplaçant les simples ancres `experiences.html#catalinas`/`#bat` par du contenu approfondi (contenu factuel repris et reformulé depuis la FAQ existante, pas dupliqué mot pour mot). 25 URLs au total dans `sitemap.xml` — **sur `dev` seulement** (voir ci-dessous), toujours 23 sur `main`.

**Mise à jour 2026-07-28** : `main` a absorbé la refonte `private-charters.html` (partenariat catamaran Playgrounds, titre EN devenu « Private Boat Charters in Tamarindo, Costa Rica \| Salty Horizon × Playgrounds ») et une nouvelle section « Las Catalinas » à fond navy sur la home. Vérifié par lecture directe d'`origin/main` (pas du répertoire de travail local, qui contient des fichiers non commités) : `catalina-islands-diving.html`/`bat-islands-diving.html` restent absents de `main` (fichiers non suivis même en local, `sitemap.xml`/`llms.txt` de `main` ne les référencent pas — correct). **Mais** `private-charters.html` sur `main` contient depuis la refonte 2 liens `<a href>` cassés dans son dropdown de nav (`catalina-islands-diving.html`, `bat-islands-diving.html`) — 404 en prod, propres à cette page (le reste du site utilise toujours les ancres `experiences.html#catalinas`/`#bat`). Voir `SEO_AUDIT_LOG.md` 2026-07-28 pour le détail. **`private-charters.html` FR/ES n'a pas suivi la refonte EN** : 4 des commits de contenu du 2026-07-28 n'ont touché que la version EN, laissant FR/ES avec 4 sections `h2` contre 6 en EN, et un `<title>`/`<h1>`/meta description toujours sans mention Playgrounds.
...
| `private-charters.html` | `/private-charters.html` | Private Boat Charters in Tamarindo, Costa Rica \| Salty Horizon × Playgrounds (EN) — titre FR/ES non mis à jour, voir note 2026-07-28 | Charters privés à bord du catamaran partenaire Playgrounds (plongée, snorkeling, paddle, e-foil, jet ski, observation des baleines, surf) |
...
- **Cache-busting** : paramètre `?v=N` sur les liens CSS/JS/certaines images. D'après l'historique Git, ce numéro est incrémenté juste avant un push vers `main` (commits dédiés « Bump cache-busting versions… »), pas à chaque commit local. **Depuis le 2026-07-17**, le JS est réparti sur 2 fichiers par page : `js/core.js?v=1` (logique, commun aux 21 pages) + `js/i18n-en.js?v=1`/`-fr`/`-es` (dictionnaire de traduction, un seul par page selon sa langue) — a remplacé l'unique `js/script.js` (supprimé). `css/styles.css` reste inchangé dans son principe.
...
9. **`private-charters.html` FR/ES en retard sur la refonte EN** (2026-07-28) : 4 sections `h2` contre 6 en EN (section vie marine et section « groupes mixtes » absentes), `<title>`/`<h1>`/meta description sans mention Playgrounds. Nécessite un travail de traduction (contenu à porter, pas une simple correction de code) — voir `SEO_AUDIT_LOG.md`.
10. **2 liens internes cassés** dans le dropdown de nav de `private-charters.html` (`main`) vers `catalina-islands-diving.html`/`bat-islands-diving.html`, pages non publiées — correction triviale (revenir aux ancres `experiences.html#catalinas`/`#bat` comme partout ailleurs sur le site) mais pas encore appliquée (mission d'audit, pas de correction). Voir `SEO_AUDIT_LOG.md`.
11. **Section « Las Catalinas » (home) sans fond navy en FR/ES** : classe CSS `section--dark` présente sur `index.html`, absente sur `fr/index.html`/`es/index.html` — seul écart structurel trouvé entre les 3 langues sur toute la home. Voir `SEO_AUDIT_LOG.md`.
