...
**Mise à jour 2026-07-28** : `main` a absorbé la refonte `private-charters.html` (partenariat catamaran Playgrounds, titre EN devenu « Private Boat Charters in Tamarindo, Costa Rica \| Salty Horizon × Playgrounds ») et une nouvelle section « Las Catalinas » à fond navy sur la home. `catalina-islands-diving.html`/`bat-islands-diving.html` restent absents de `main` (fichiers non suivis même en local, `sitemap.xml`/`llms.txt` de `main` ne les référencent pas — correct). Les 2 liens `<a href>` cassés qui pointaient vers ces pages depuis le dropdown de nav de `private-charters.html` ont été corrigés (retour aux ancres `experiences.html#catalinas`/`#bat`, cohérent avec le reste du site). **`private-charters.html` FR/ES a depuis été reconstruit pour une parité structurelle complète avec EN** (mêmes sections, même nombre de balises, `<title>`/`<h1>`/meta description mentionnant désormais Playgrounds) — voir `SEO_AUDIT_LOG.md` 2026-07-28 pour le détail des 3 passes de correction.
...
| `private-charters.html` | `/private-charters.html` | Private Boat Charters in Tamarindo, Costa Rica \| Salty Horizon × Playgrounds (titre FR/ES équivalent depuis le 2026-07-28) | Charters privés à bord du catamaran partenaire Playgrounds (plongée, snorkeling, paddle, e-foil, jet ski, observation des baleines, surf) |
