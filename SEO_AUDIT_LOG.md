...
**Décisions** : le reste du Lot 2 (base des prix, `sameAs`/adresse, auteurs du blog, badge « 12+ ans », compteur d'avis) et le reste du Lot 3 (titres/descriptions, 404, H1 produit, blog, images en double) ne sont pas souhaités pour le moment. Recadrages des images de partage validés par l'utilisateur ; l'accueil partage la raie manta. Les liens vers les guides Catalinas/Bat Islands (commit `114069e`) ont été retirés ensuite à la demande du propriétaire, qui retravaille ces guides (les pages restent en ligne) ; la photo du corps de la page Snorkeling a été remplacée par celle du snorkeler de la home. L’en-tête photo des 5 pages d’expérience (Discover, Intro, Fun Dive Catalinas, Fun Dive Bat Islands, Snorkeling) est masqué pour l’instant, en attendant une refonte du design (photos de qualité insuffisante à cette largeur) : une règle CSS le rend visuellement invisible (même technique que `.sr-only`), donc le `<h1>` et le fil d’Ariane restent dans le DOM pour le SEO et les lecteurs d’écran ; l’en-tête du site y est plein dès le haut de page (sinon logo blanc sur fond blanc, ce qui corrige aussi le logo invisible en haut de page sur mobile) ; préchargement d’image et `fetchpriority="high"` passent sur la photo du corps de page. Pour rétablir le bandeau : supprimer le bloc CSS commenté « Experience pages: banner hidden for now » dans `css/styles.css`. Inchangé : `padi-courses`, `private-charters`, guides et FAQ gardent leur photo d’en-tête. Les 6 pages guide Catalinas/Bat Islands (EN/FR/ES) sont retirées de la production (`main`) à la demande du propriétaire, qui les retravaille dans `dev` (elles y restent, en `index, follow`) ; retirées aussi de `sitemap.xml` (44 à 38 URLs) et de `llms.txt`, et le lien « Learn more » des 6 pages `fun-dive-*` est retiré sur `main` seulement.
